...
const heads = ["Name", "NPWP Number", "Address", "City", "State", "Zip Code", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('settings.company.branch.getdata'), {
        params: {
            page: page,
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        <div class="flex flex-col md:flex-row md:-mr-px">
            ...
            <div class="grow overflow-scroll">
                <!-- Panel Header -->
                <div class="border-b">
                    <h2 class="text-2xl text-slate-800 font-bold p-6">Branches</h2>
                </div>
                    ...
                    <section>
                        <header class="block justify-between items-center sm:flex py-6">
                            <h2 class="font-semibold text-slate-800">
                                All Branches <span class="text-slate-400 !font-medium ml">{{ pagination.total
                                }}</span>
                            </h2>
                            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                                <!-- Filter -->
                                <VFilter @search="searchHandle" />
                                <VButton label="Add Branch" type="primary" @click="handleAddModalForm"
                                    class="mt-auto" />
                            </div>
                        </header>
                        ...
                        <VDataTable :heads="heads" :isLoading="isLoading" wrapperClass="!px-0">
                            ...
                            <tr v-for="(data, index) in query" :key="index" v-else>
                                ...
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.npwp_number ?? '-' }} </td>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.address }} </td>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.city }} </td>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.state }} </td>
                                ...
                                <td class="px-4 whitespace-nowrap h-16 text-right">
                                    <VDropdownEditMenu class="relative inline-flex r-0" :align="'right'"
                                        ...
                                        <li class="cursor-pointer hover:bg-slate-100" @click="alertDelete(data)" v-if="data.id > 1">
                                            <div class="flex items-center space-x-2 p-3">
                                                <span>
                                                    <VTrash color="danger" />
                                                </span>
