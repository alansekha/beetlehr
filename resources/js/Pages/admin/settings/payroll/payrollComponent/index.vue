...
const getData = debounce(async (page) => {
    axios.get(route('settings.payroll.components.getdata'), {
        params: {
            page: page,
            search: searchFilter.value,
            filter_branch: filter.value.branch_id,
            filter_designation: filter.value.designation_id
        }
    }).then((res) => {
...
const handleSetValue = (data) => {
    Inertia.visit(route('settings.payroll.components.setvalue', { 'id': data.id }));
}
...
const deleteHandle = async () => {
    axios.delete(route('settings.payroll.components.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        <div class="flex flex-col md:flex-row md:-mr-px">
            ...
            <div class="grow overflow-scroll">
                <!-- Panel Header -->
                <div class="border-b">
                    <h2 class="text-2xl text-slate-800 font-bold p-6">Payroll Components</h2>
                </div>
                    ...
                    <section>
                        <header class="block justify-between items-center sm:flex py-6">
                            <h2 class="font-semibold text-slate-800">
                                All Payroll Components <span class="text-slate-400 !font-medium ml">{{ pagination.total
                                }}</span>
                            </h2>
                            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                                ...
                                <VButton label="Add Component" type="primary" @click="handleAddModalForm"
                                    class="mt-auto" />
                            </div>
                        </header>
                        ...
                        <VDataTable :heads="heads" :isLoading="isLoading" wrapperClass="!px-0">
                            ...
                            <tr v-for="(data, index) in query" :key="index" v-else>
                                ...
                                <td class="px-4 whitespace-nowrap h-16 capitalize">
                                    <VBadge :text="data.is_editable ? 'yes' : 'no'"
                                        :color="data.is_editable ? 'success' : 'primary'" size="sm" />
                                </td>
                                <td class="px-4 whitespace-nowrap h-16 capitalize">
                                    <VBadge :text="data.is_taxable ? 'yes' : 'no'"
                                        :color="data.is_taxable ? 'success' : 'primary'" size="sm" />
                                </td>
                                <td class="px-4 whitespace-nowrap h-16 text-right">
                                    <VDropdownEditMenu class="relative inline-flex r-0" :align="'right'"
                                        ...
                                        <li class="cursor-pointer hover:bg-slate-100" @click="handleSetValue(data)">
