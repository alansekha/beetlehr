...
const breadcrumb = [
    ...
    },
    {
        ...
        active: false,
        to: route('settings.payroll.employee-base-salaries.index')
    },
    {
        name: "Employee Base Salaries",
        active: true,
        to: route('settings.payroll.employee-base-salaries.index')
    },
]
...
const heads = ["Name", "Salary Type", "Amount", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('settings.payroll.employee-base-salaries.getdata'), {
        params: {
            page: page,
...
const deleteHandle = async () => {
    axios.delete(route('settings.payroll.employee-base-salaries.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        <div class="flex flex-col md:flex-row md:-mr-px">
            ...
            <div class="grow overflow-scroll">
                <!-- Panel Header -->
                <div class="border-b">
                    <h2 class="text-2xl text-slate-800 font-bold p-6">Employee Base Salaries</h2>
                </div>
                    ...
                    <section>
                        <header class="block justify-between items-center sm:flex py-6">
                            <h2 class="font-semibold text-slate-800">
                                All Employee Base Salaries <span class="text-slate-400 !font-medium ml">{{
                                        pagination.total
                                }}</span>
                            </h2>
                            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                                ...
                                <VButton label="Generate Salaries" type="primary" @click="handleAddModalForm"
                                    class="mt-auto" />
                            </div>
                        </header>
                        ...
                        <VDataTable :heads="heads" :isLoading="isLoading" wrapperClass="!px-0">
                            ...
                            <tr v-for="(data, index) in query" :key="index" v-else>
                                ...
                                <td class="px-4 whitespace-nowrap h-16 capitalize"> {{ data.salary_type }} </td>
                                <td class="px-4 whitespace-nowrap h-16"> Rp{{ data.amount_formatted }} </td>
