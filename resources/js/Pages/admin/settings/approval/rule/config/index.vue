...
const breadcrumb = [
    ...
    },
    {
        name: "Config Rule",
        active: true,
        to: route('settings.approval.rule.config', { 'id': props.additional.data.id })
    },
]
...
const heads = ["Name", "Need Approval", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('settings.approval.rule.config.getdata'), {
        params: {
            page: page,
...
const deleteHandle = async () => {
    axios.delete(route('settings.approval.rule.config.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        <div class="flex flex-col md:flex-row md:-mr-px">
            ...
            <div class="grow overflow-scroll">
                <!-- Panel Header -->
                <div class="border-b">
                    <h2 class="text-2xl text-slate-800 font-bold p-6">{{ additional.data.label }} Configuration</h2>
                </div>
                    ...
                    <section>
                        <header class="block justify-between items-center sm:flex py-6">
                            <h2 class="font-semibold text-slate-800">
                                All Branch Configuration <span class="text-slate-400 !font-medium ml">{{
                        ...
                        <VDataTable :heads="heads" :isLoading="isLoading" wrapperClass="!px-0">
                            ...
                            <tr v-for="(data, index) in query" :key="index" v-else>
                                ...
                                <td class="px-4 whitespace-nowrap h-16 capitalize">
                                    <VBadge :text="data.need_approval_formatted"
                                        :color="data.need_approval_formatted === 'yes' ? 'success' : 'danger'" size="sm"
                                        v-if="(data.need_approval !== null)" />
                                    <div v-else>
                                        {{ data.need_approval_formatted }}
                                    </div>
                                </td>
