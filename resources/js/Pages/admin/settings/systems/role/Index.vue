...
const getData = debounce(async (page) => {
    axios.get(route('settings.systems.role.getdata'), {
        params: {
            page: page,
...
const handleAddRole = () => {
    Inertia.visit(route('settings.systems.role.createpage'));
}

const handleEditRole = (data) => {
    Inertia.visit(route('settings.systems.role.editpage', {'id': data.id}));
}
...
const deleteRoleManagement = async () => {
    axios.delete(route('settings.systems.role.deleterole', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
