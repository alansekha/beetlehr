...
const breadcrumb = [
    ...
    {
        name: "Leave Management Settings",
        active: false,
        to: route('settings.leave.quota.index')
    },
    {
        name: "Leave Quota",
        active: true,
...
const heads = ["Leave Type", "Employee Name", "Quota", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('settings.leave.quota.getdata'), {
        params: {
            page: page,
...
const deleteHandle = async () => {
    axios.delete(route('settings.leave.quota.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        <div class="flex flex-col md:flex-row md:-mr-px">
            ...
            <div class="grow overflow-scroll">
                <!-- Panel Header -->
                <div class="border-b">
                    <h2 class="text-2xl text-slate-800 font-bold p-6">Leave Quota</h2>
                </div>
                    ...
                    <section>
                        <header class="block justify-between items-center sm:flex py-6">
                            <h2 class="font-semibold text-slate-800">
                                All Leave Quotas <span class="text-slate-400 !font-medium ml">{{ pagination.total
                                }}</span>
                            </h2>
                            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                                ...
                                <VButton label="Generate Quota" type="primary" @click="handleAddModalForm"
                                    class="mt-auto" />
                            </div>
                        </header>
                        ...
                        <VDataTable :heads="heads" :isLoading="isLoading" wrapperClass="!px-0">
                            ...
                            <tr v-for="(data, index) in query" :key="index" v-else>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.leave_type_name }} </td>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.employee_name }} </td>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.quota }} </td>
