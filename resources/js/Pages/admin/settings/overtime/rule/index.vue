...
const breadcrumb = [
    ...
    },
    {
        ...
        active: false,
        to: route('settings.overtime.rule.index')
    },
    {
        name: "Rules",
        active: true,
...
const heads = ["Min Clockin", "Max Clockout", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('settings.overtime.rule.getdata'), {
        params: {
            page: page,
...
const deleteHandle = async () => {
    axios.delete(route('settings.overtime.rule.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
...
<template>
    ...
    <div class="mb-4 sm:mb-6 flex justify-between items-center">
        <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Overtime Settings</h1>
    </div>
    <!-- Content -->
                    ...
                    <section>
                        <header class="block justify-between items-center sm:flex py-6">
                            <h2 class="font-semibold text-slate-800">
                                All Rules <span class="text-slate-400 !font-medium ml">{{ pagination.total }}</span>
                            </h2>
                            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                                <!-- Filter -->
                                <VFilter @search="searchHandle"/>
                                <VButton label="Add Rule" type="primary" @click="handleAddModalForm" class="mt-auto"/>
                            </div>
                        </header>
                        ...
                        <VDataTable :heads="heads" :isLoading="isLoading" wrapperClass="!px-0">
                            ...
                            <tr v-for="(data, index) in query" :key="index" v-else>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.clock_in_format }} </td>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.clock_out_format }} </td>
