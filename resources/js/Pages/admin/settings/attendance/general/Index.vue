...
const formError = ref({})
const form = reactive({
    close_break_page: props.additional.data.close_break_page === "1" ? true : false,
})


const submit = () => {
    submitLoading.value = true
    axios.post(route('settings.attendance.general.updateCloseBreakup'), {
        close_break_page: form.close_break_page,
    }).then((res) => {
        notify({
            type: "success",
            group: "top",
...
const discard = () => {
    form.close_break_page = props.additional.data.close_break_page === "1" ? true : false
}
                ...
                <div class="p-6 space-y-6">
                    ...
                    <section>
                        <div class="sm:flex sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 mt-3">
                            <div class="sm:w-full">
                                <h3 class="text-xl leading-snug text-slate-800 font-bold mb-1">Close Breakup</h3>
                                <div class="text-sm text-slate-500 mb-4">Turn it on if you want set user can exit the
                                    break page when it is on breaktime in the mobile application</div>
                                <VSwitch v-model="form.close_break_page" label="Action Close" type="block"
                                    :tooltip="false" />
                            </div>
                        </div>
                    </section>
