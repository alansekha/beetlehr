...
const breadcrumb = [
    ...
    },
    {
        ...
        active: false,
        to: route('settings.employee.designation.index')
    },
    {
        name: "Employment Status",
        active: true,
        to: route('settings.employee.status.index')
    },
]
...
const heads = ["Name", "Employment Type","Status",""]
...
const getData = debounce(async (page) => {
    axios.get(route('settings.employee.status.getdata'), {
        params: {
            page: page,
...
const deleteEmployeeGroup = async () => {
    axios.delete(route('settings.employee.status.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
...
<template>
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        <div class="flex flex-col md:flex-row md:-mr-px">
            ...
            <div class="grow overflow-scroll">
                <!-- Panel Header -->
                <div class="border-b">
                    <h2 class="text-2xl text-slate-800 font-bold p-6">Employment Status</h2>
                </div>
                    ...
                    <section>
                        <header class="block justify-between items-center sm:flex py-6">
                            <h2 class="font-semibold text-slate-800">
                                All Employment Status <span class="text-slate-400 !font-medium ml">{{ pagination.total
                                }}</span>
                            </h2>
                            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                                <!-- Filter -->
                                <VFilter @search="searchGroup" />
                                <VButton label="Add Employment Status" type="primary" @click="handleAddModalForm" class="mt-auto"/>
                            </div>
                        </header>
                        ...
                        <VDataTable :heads="heads" :isLoading="isLoading" wrapperClass="!px-0">
                            ...
                            <tr v-for="(data, index) in query" :key="index" v-else>
                                ...
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.employment_type }} ( {{ data.pkwt_type}} ) </td>
                                <td class="px-4 whitespace-nowrap h-16 capitalize"> <VBadge :text="data.status ? `Active` : `Inactive` " :color="getTypeStatus(data.status)" size="sm" /> </td>
    ...
    <VModalForm :data="itemSelected" :update-action="updateAction" :open-dialog="openModalForm" @close="closeModalForm"
        @successSubmit="successSubmit" ref="modalForm" :additional="additional" />
    <VAlert :open-dialog="openAlert" @closeAlert="closeAlert" @submitAlert="deleteEmployeeGroup" type="danger"
