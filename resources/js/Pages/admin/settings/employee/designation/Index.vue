...
const getData = debounce(async (page) => {
    axios.get(route('settings.employee.designation.getdata'), {
        params: {
            page: page,
...
const deleteEmployeeGroup = async () => {
    axios.delete(route('settings.employee.designation.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        <div class="flex flex-col md:flex-row md:-mr-px">
            ...
            <div class="grow overflow-scroll">
                <!-- Panel Header -->
                <div class="border-b">
                    <h2 class="text-2xl text-slate-800 font-bold p-6">Designation</h2>
                </div>
                    ...
                    <section>
                        <header class="block justify-between items-center sm:flex py-6">
                            <h2 class="font-semibold text-slate-800">
                                All Designation Group <span class="text-slate-400 !font-medium ml">{{ pagination.total
                                }}</span>
                            </h2>
                            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                                <!-- Filter -->
                                <VFilter @search="searchGroup" />
                                <VButton label="Add Desigation" type="primary" @click="handleAddModalForm" class="mt-auto"/>
                            </div>
                        </header>
